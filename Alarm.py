...
cfg = [0,0]
i = 0
File = open('Config.txt', 'r') #открываем файл по пути Config.txt и с аргументом 'r' что означает readonly
for line in File: #разбиваем построчно 
    cfg[i] = line[19:29].strip()# берем лишь символы под номером 19-29
    i  = i+1 
y = float(cfg[0].replace(',','.')) #разница, рублей, которая была залита массивом с заменой запятую на точку
x = int(cfg[1]) #Время между проверками, минут 
